...
export default function ClubDetailsPage() {
  ...
  const fetchClubDetails = async () => {
    try {
      ...
      // 2. Fetch slots for this club
      const { data: slots, error: slotsError } = await supabase.from('slots').select('*').eq('club_id', id).eq('status', 'ACTIVE');
  ...
  const { club, totalCapacity, totalEnrolled, students } = data;
  
  const pieData = [
    { name: 'Enrolled', value: totalEnrolled, color: '#3b82f6' },
    { name: 'Available Slots', value: Math.max(0, totalCapacity - totalEnrolled), color: '#e2e8f0' }
  ];

  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Link href="/overall-clubs/clubs" className="p-2 bg-white border border-slate-200 hover:bg-slate-50 rounded-full transition-colors shadow-sm">
          <ArrowLeft className="w-5 h-5 text-slate-600" />
        </Link>
        <div>
          <h1 className="text-2xl font-bold text-slate-900">{club.name}</h1>
          <p className="text-slate-500 mt-1">Club Details & Current Status</p>
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Left Column: Club Info & Pie Chart */}
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  {club.faculty_mobile ? (
                    <a href={`tel:${club.faculty_mobile}`} className="text-blue-600 font-medium hover:underline">
                      {club.faculty_mobile}
                    </a>
                  ) : (
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
            ...
            <div className="text-center mt-2 p-3 bg-slate-50 rounded-lg border border-slate-100">
              <p className="text-sm font-medium text-slate-600">
                <span className="text-blue-600 font-bold">{totalEnrolled}</span> enrolled out of <span className="font-bold">{totalCapacity}</span> capacity
              </p>
            </div>
          </div>
        </div>
                    ...
                    <tr>
                      <td colSpan={4} className="px-6 py-16 text-center">
                        <div className="flex flex-col items-center justify-center">
                          ...
                          <p className="text-slate-500 font-medium">No students are currently enrolled in this club.</p>
                        </div>
                      </td>
                    </tr>
                  )}
