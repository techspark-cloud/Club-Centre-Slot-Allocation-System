...
import { getClubsAllocations } from './actions';
...
export default function OverallClubsStudents() {
  ...
  useEffect(() => {
    async function fetchData() {
      const data = await getClubsAllocations();
  ...
  const filteredData = allocations.filter(a => {
    ...
    const matchesSearch = (
      ...
      a.slots?.clubs?.name?.toLowerCase().includes(term)
    );
  ...
  const downloadCSV = () => {
    ...
    const headers = ['Register Number', 'Student Name', 'Course', 'Semester', 'Section', 'Gender', 'Contact', 'Club Name', 'Day', 'Session', 'Venue'];
    const csvContent = [
      headers.join(','),
      ...filteredData.map(a => [
    ...
    link.setAttribute('download', 'Overall_Clubs_Students_Report.csv');
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-2xl font-black text-slate-900 tracking-tight">Clubs Student Directory</h2>
          <p className="text-slate-500 mt-1">Advanced filtering for student allocation tracking.</p>
        </div>
        <button
          onClick={downloadCSV}
          disabled={filteredData.length === 0}
          className="flex items-center justify-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-xl font-bold hover:bg-blue-700 transition-colors disabled:opacity-50 text-sm whitespace-nowrap"
        >
          ...
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50 text-slate-600 font-bold border-b border-slate-200">
              <tr>
                ...
                <th className="px-6 py-4">Club Assigned</th>
                  ...
                  <tr key={a.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <span className="inline-flex items-center px-2.5 py-1 rounded-md text-xs font-bold bg-blue-50 text-blue-700 border border-blue-100">
                        {a.slots?.clubs?.name || 'Unknown Club'}
                      </span>
                    </td>
                    <td className="px-6 py-4">
