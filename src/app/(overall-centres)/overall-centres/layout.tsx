...
import OverallCentresNavigation from './OverallCentresNavigation';

export default async function OverallCentresLayout({ children }: { children: ReactNode }) {
  const supabase = await createClient();
  const { data: { user } } = await supabase.auth.getUser();

  if (!user || user.email !== 'centres@rit.edu') {
    redirect('/login');
  }
        ...
        <div className="w-full px-4 sm:px-6 lg:px-6">
          <div className="flex justify-between h-16 items-center relative">
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-xl font-black text-slate-900 tracking-tight">Overall Centres Portal</h1>
            </div>
