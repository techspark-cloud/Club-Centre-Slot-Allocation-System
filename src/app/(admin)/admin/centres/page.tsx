...
export default function CentresPage() {
  ...
  const saveEdit = async (centreId: string) => {
    ...
    if (!error) {
      if (facultyMobileStr && facultyNameStr) {
        ...
        await syncFacultyAuthUser(facultyNameStr, facultyMobileStr, 'CENTRE', centreId);
      }
  ...
  const viewMembers = async (centre: any) => {
  ...
  const downloadOverallPDF = async () => {
    ...
    try {
      ...
      doc.text("OVERALL CENTRES ENROLLMENT REPORT", pageWidth / 2, 45, { align: "center" });
      ...
      // Summary Analysis
      const summaryStats = Array.from(centreMap.entries()).map(([id, name]) => {
        const count = sortedAllocations.filter(a => a.slot.centre_id === id).length;
        return { name, count };
      }).filter(s => s.count > 0).sort((a, b) => b.count - a.count);

      const summaryTableColumn = ["S.No", "Centre Name", "Total Enrolled"];
      ...
      const tableColumn = ["S.No", "Centre Name", "Register No", "Student Name", "Course", "Day"];
      const tableRows = sortedAllocations.map((m, i) => [
        i + 1,
        centreMap.get(m.slot.centre_id) || '',
        m.student?.register_no || '',
        m.student?.name || '',
        `${m.student?.course || ''} - ${m.student?.section || ''}`,
        m.slot?.day || ''
      ...
      doc.save(`Overall_Centres_Enrollment_Report.pdf`);
    } catch (e) {
      console.error(e);
          ...
          <div className="bg-white rounded-3xl w-full max-w-4xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden animate-in zoom-in-95 duration-300">
            ...
            <div className="p-6 sm:p-8 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className="text-xl sm:text-2xl font-black text-slate-900 tracking-tight">{selectedCentre?.name}</h2>
            ...
            <div className="flex-1 overflow-auto p-6 sm:p-8 bg-slate-50/30">
              {loadingMembers ? (
                ...
                <div className="text-center py-16 bg-white rounded-2xl border-2 border-dashed border-slate-200">
                  ...
                  <p className="text-slate-500 font-bold">No students have enrolled in this centre yet.</p>
                </div>
              ) : (
                      ...
                      <tbody className="divide-y divide-slate-100">
                        {members.map((m, idx) => (
                          <tr key={idx} className="hover:bg-indigo-50/50 transition-colors">
                            ...
                            <td className="px-5 py-4 font-bold text-indigo-600 text-sm text-right">{m.slot?.day}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
            ...
            <div className="p-6 border-t border-slate-100 bg-white flex items-center justify-between">
              ...
              <div className="flex gap-3">
                {members.length > 0 && (
                  <button 
                    onClick={downloadPDF}
                    className="flex items-center gap-2 px-5 py-2.5 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 border-2 border-indigo-200 rounded-xl font-bold text-sm transition-colors"
                  >
