...
import { Building2, Phone, User, Pencil, Check, X, Plus, Users, Download, Loader2, Copy, FileSpreadsheet } from 'lucide-react';
...
export default function ClubsPage() {
  ...
  const startEdit = (club: any) => {
    ...
    setEditForm({ description: club.description || '', faculty_name: club.faculty_name || '', faculty_mobile: club.faculty_mobile || '' });
  };
  ...
  const downloadOverallExcel = async () => {
    ...
    try {
      ...
      const excelData = sortedAllocations.map((m, i) => ({
        "S.No": i + 1,
        "Club Name": clubMap.get(m.slot.club_id) || '',
      ...
      XLSX.utils.book_append_sheet(workbook, worksheet, "Overall Clubs Report");
      XLSX.writeFile(workbook, "Overall_Clubs_Enrollment_Report.xlsx");
              ...
              <div className="mt-4 pt-4 border-t-2 border-slate-100">
                {editingId === club.id ? (
                  <div className="space-y-3">
                    <div>
                      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1">Club Description</label>
                      <textarea
                        value={editForm.description}
                        onChange={e => setEditForm(f => ({ ...f, description: e.target.value }))}
                        placeholder="e.g. A creative media community..."
                        rows={2}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="mt-4 bg-slate-50 p-3 rounded-xl border-2 border-slate-100">
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 flex items-center justify-between">
                          ...
                          <button 
                            onClick={() => {
                              const creds = `Login ID: ${club.faculty_mobile?.trim()}@rit.faculty\nPassword: ${club.faculty_mobile?.trim()}`;
                        ...
                        <div className="space-y-1 text-xs font-mono font-bold text-slate-700">
                          <p className="flex flex-wrap items-center gap-1">
                            ...
                            <span className="break-all">{club.faculty_mobile?.trim()}@rit.faculty</span>
                          </p>
                          <p className="flex items-center gap-1">
                            <span className="text-slate-400">PW:</span> 
                            <span>{club.faculty_mobile?.trim()}</span>
                          </p>
                        </div>
                      </div>
                    </div>
