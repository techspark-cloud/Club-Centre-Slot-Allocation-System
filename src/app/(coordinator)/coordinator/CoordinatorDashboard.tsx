...
import { Users, Calendar, MapPin, Download, BookOpen, Clock, Building2, User, CheckCircle2, XCircle, AlertCircle, FileText, QrCode } from 'lucide-react';
...
export default function CoordinatorDashboard({
  ...
  return (
    <div className="w-full space-y-8 animate-in fade-in duration-500">
      ...
      {/* Navigation Tabs */}
      <div className="flex flex-wrap bg-slate-200/50 p-1.5 rounded-2xl w-fit gap-1">
        ...
        <button
          ...
          className={`px-6 py-2.5 rounded-xl text-sm font-black uppercase tracking-widest transition-all flex items-center gap-2 ${
            activeTab === 'reports' ? 'bg-blue-900 text-white shadow-sm' : 'text-slate-500 hover:text-slate-700'
          }`}
        >
        ...
        {activeTab === 'reports' ? (
            ...
            <div className="bg-gradient-to-r from-slate-900 via-blue-950 to-slate-900 p-8 rounded-3xl text-white border border-slate-800 shadow-xl relative overflow-hidden">
              ...
              <div className="relative z-10">
                <span className="text-[10px] font-black uppercase tracking-widest bg-blue-500/20 text-blue-300 px-3 py-1 rounded-full border border-blue-400/30">
                  OFFICIAL INSTITUTIONAL REPORTS & AUDIT PORTAL
                </span>
                <h2 className="text-3xl font-black uppercase tracking-wider text-white mt-3">
                  My Extracurricular Audit Reports
                </h2>
                <p className="text-slate-300 text-sm font-medium mt-2 max-w-2xl">
                  Access official attendance audit reports, compliance statistics, and visual PDF documents for your assigned clubs and centres.
                </p>
              </div>
            </div>

            {/* List of Assigned Entities & their Report Links */}
            {allAssignedEntities.length === 0 ? (
              <div className="text-center py-16 bg-white rounded-3xl border-2 border-dashed border-slate-200">
                <FileText className="w-12 h-12 text-slate-300 mx-auto mb-3" />
                <p className="text-slate-500 font-bold text-base">No assigned clubs or centres found.</p>
              </div>
            ) : (
                  ...
                  <div key={entity.id} className="bg-white rounded-3xl border-2 border-slate-200 shadow-sm overflow-hidden p-6 sm:p-8 space-y-6">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b-2 border-slate-100 pb-6">
                      ...
                      <div className="flex items-center gap-3">
                        <Link
                          href={`/admin/audit-reports/print/${encodeURIComponent(entity.name)}`}
                          target="_blank"
                          className="flex items-center gap-2 bg-blue-900 hover:bg-blue-800 text-white font-bold px-6 py-3 rounded-xl transition-all shadow-md text-sm shrink-0"
                        >
                          <FileText className="w-4 h-4 text-blue-300" />
                    ...
                    {/* Audit Index Table preview */}
                    <div className="space-y-3">
                      <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest flex items-center justify-between">
                        <span>📊 Scheduled Session Audit Records</span>
                        <span className="text-slate-500 font-bold">{entityReports.length} records</span>
                      </h4>

                      {isAuditLoading ? (
                        <div className="p-8 text-center text-slate-400 font-bold text-sm">Loading activity audit records...</div>
          ...
          return (
            <div key={entity.id} className="bg-white rounded-3xl border-2 border-slate-100 shadow-sm overflow-hidden">
              <div className="p-6 sm:p-8 border-b-2 border-slate-100 bg-slate-50/50 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                ...
                <Link 
                  href={`/admin/audit-reports/print/${encodeURIComponent(entity.name)}`}
                  target="_blank"
                  className="inline-flex items-center justify-center gap-2 bg-gradient-to-r from-blue-900 to-indigo-900 hover:from-blue-800 hover:to-indigo-800 text-white font-bold px-5 py-2.5 rounded-xl transition-all shadow-md text-xs sm:text-sm shrink-0 border border-blue-800/40"
                >
                  <FileText className="w-4 h-4 text-blue-300" />
